...
// WAŻNE: Importujemy tło, żeby pokój był widoczny też tutaj
import { HoloBackground } from "@/app/components/features/HoloBackground";

type Language = "pl" | "en";

// --- DANE (Przeniesione tutaj dla porządku) ---
      ...
      <div className="relative rounded-2xl border border-white/10 bg-zinc-950 overflow-hidden shadow-2xl mt-8 mb-20 group/card animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="grid lg:grid-cols-2 min-h-[600px]">
          ...
          <div className="relative bg-black flex flex-col overflow-hidden h-full min-h-[500px]">
            ...
            />
            {viewMode === "showroom" ? (
              <div className="relative w-full h-full flex flex-col group/gallery">
                <div
                ...
                <button
                  onClick={prevSlide}
                  className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 text-white rounded-full opacity-0 group-hover/gallery:opacity-100 transition-opacity"
                >
                  <ChevronLeft />
                </button>
                <button
                  onClick={nextSlide}
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/50 text-white rounded-full opacity-0 group-hover/gallery:opacity-100 transition-opacity"
                >
                  <ChevronRight />
                </button>
              </div>
...
// --- GŁÓWNY EXPORT TEGO PLIKU ---
interface ProjectDetailsProps {
  lang: Language;
  onBack: () => void;
}

export function ProjectDetails({ lang, onBack }: ProjectDetailsProps) {
  return (
    <>
      {/* 1. TŁO - To sprawia, że jesteśmy "w pokoju" */}
      <div className="fixed inset-0 z-0">
        <HoloBackground />
        {/* Ciemna warstwa, żeby tekst był czytelny */}
